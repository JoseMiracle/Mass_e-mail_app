...
from users.tasks import send_otp, send_welcome_mssg
...
class OtpGenerationSerializer(serializers.Serializer):
    ...
    def validate(self, data: dict[str:Any]) -> dict[str:Any]:
        ...
        if (
            user_otp.exists()
        ):  # Checks if there is an existing otp for the user and deletes it
            user_otp.delete()
        return data

    def create(self, validated_data: dict[str, Any]) -> dict[str:Any]:
        totp = pyotp.TOTP(pyotp.random_base32(), digits=6)
        OTP.objects.create(email=validated_data["email"], otp=totp.now())
